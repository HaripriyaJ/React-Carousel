        ...
        carouselData.map((prop, index) => 
          <span title={`${prop.id}`} className="slider" key={`${prop.id}`}>
            { carouselData.length < 5 || (index === 0 || index === carouselData.length - 1) ? 
              <svg width="45" height="12" xmlns="http://www.w3.org/2000/svg" xlinkTitle={`${prop.id}`} onClick={() => dispatch(sliderNavigate())}>
                <path 
              ...
              :
              <svg width="13" height="12" xmlns="http://www.w3.org/2000/svg" xlinkTitle={`${prop.id}`} onClick={() => dispatch(sliderNavigate())}>
